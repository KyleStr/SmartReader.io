...
def convert_txt_html(html_path, js):

	docs_folder = os.path.join(html_path, js[0]["folder_name"])
	...
	for k in range(len(js)):
		for i in range(len(js[k][u'summary_points'])):
			try:
				...
				print("Error on convert_txt_html (convert_dataset.py). An exception occurred: ", e)
				pass
